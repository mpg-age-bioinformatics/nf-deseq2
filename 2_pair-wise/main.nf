...
process tx2gene {
  ...
  """
    ...
    write.xlsx(res_counts, "/workdir/deseq2_output/all_res_counts.xlsx", row.names = TRUE, col.names = TRUE)
    ...
    write.xlsx(res_counts, "/workdir/deseq2_output/all_results_stats.xlsx", row.names = FALSE, col.names = TRUE)
    write.xlsx(res_counts, "/workdir/deseq2_output/all_results_stats.xlsx", row.names = FALSE, col.names = TRUE)
...
process deseq2 {
  ...
  """
    ...
    # filein
    sampleTable<-read.delim2("/workdir/deseq2_output/${input_file}",sep = "\t", row.names = 1)
    samples<-row.names(sampleTable)
...
workflow {
  tx2gene()
  data = channel.fromPath( "${params.project_folder}/deseq2_output/*.input.txt" )
